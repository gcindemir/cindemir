...
final class Cindemir_SEO_Fixes {
	...
	private static function build_singular_hreflangs() {
		...
		foreach ( $codes as $code ) {
			...
			$url = self::permalink_for_lang( $tid, $code );
			if ( ! is_string( $url ) || '' === $url ) {
				continue;
			}
	...
	/**
	 * Permalink for a post in a specific language (strip sticky front-lang pollution).
	 *
	 * @param int    $post_id Post ID.
	 * @param string $lang    Language code.
	 * @return string
	 */
	private static function permalink_for_lang( $post_id, $lang ) {
		$post_id = (int) $post_id;
	...
	private static function normalize_hreflang_url( $url, $lang = null ) {
		...
		if ( in_array( $code, array( 'en', 'en-us', 'en_us', 'x-default' ), true ) ) {
			// English / x-default must never keep a sticky ?lang=ru from the front request.
			unset( $q['lang'] );
		} elseif ( $code && empty( $q['lang'] ) ) {
			$q['lang'] = $code;
		} elseif ( ! $code && ! empty( $q['lang'] ) && in_array( $q['lang'], array( 'en', 'en-us', 'en_us' ), true ) ) {
			unset( $q['lang'] );
		}
		// Do NOT use home_url() here: WPML filters it and re-appends the front language.
		$host = 'https://cindemirlaw.com';
		$new  = $host . user_trailingslashit( $path );
		if ( ! empty( $q ) ) {
	...
	private static function lcp_image_needles( $html = '' ) {
		$needles = array();
		$is_home = self::request_is_home_page( $html );
		$is_team = self::request_is_team_about_page( $html );
		$lcp     = self::lcp_preload_image();
	...
	private static function request_is_home_page( $html = '' ) {
		...
		if ( is_string( $html ) && (bool) preg_match( '/<body\b[^>]*\bclass=(["\'])[^"\']*\b(?:home|page-id-(?:15|2570|2568))\b/i', $html ) ) {
			return true;
		}
		$uri  = isset( $_SERVER['REQUEST_URI'] ) ? (string) $_SERVER['REQUEST_URI'] : '';
	...
	private static function request_is_team_about_page( $html = '' ) {
		if ( function_exists( 'is_page' ) && is_page( array( 19, 2427, 16, 2 ) ) ) {
			return true;
		}
	...
	public static function pagespeed_optimize_lcp_html( $html ) {
		...
		// Ensure a single LCP winner even if this runs twice (ob + rocket_buffer).
		$html = preg_replace( '/\sfetchpriority=(["\'])high\1/i', '', $html );
		if ( ! is_string( $html ) ) {
			return '';
		}

		$needles      = self::lcp_image_needles( $html );
		$is_home      = self::request_is_home_page( $html );
		...
		$html = preg_replace_callback(
			...
			static function ( $m ) use ( &$lcp_set, $needles, $is_home, $is_team_page, $team_pref ) {
				...
				$tag = $im[0];
				$noscript_img = '';
				if ( preg_match( '#<noscript>\s*(<img\b[^>]*>)\s*</noscript>#i', $full, $nm ) ) {
					$noscript_img = $nm[1];
				}
				$blob = $tag . ' ' . $noscript_img;

				if ( preg_match( '/\bcindemir-lang-flag\b/i', $blob ) ) {
					return $full;
				}
				if ( preg_match( '#\b(?:src|data-lazy-src|data-src)=(["\'])[^"\']*(?:sitepress|/flags/|res/flags|uploads/flags/)[^"\']*\1#i', $blob ) ) {
					$tag = preg_replace( '/\sfetchpriority=(["\'])[^"\']*\1/i', '', $tag );
					if ( ! preg_match( '/\bloading=/i', $tag ) ) {
						$tag = preg_replace( '/<img\b/i', '<img loading="lazy"', $tag, 1 );
					}
					return $tag . ( $noscript_img ? '<noscript>' . $noscript_img . '</noscript>' : '' );
				}

				$is_logo = (bool) preg_match( '/\b(?:logo|cropped-logoicon|cindemir-site-brand)\b/i', $blob );
				// On home/about/team the logo must never steal LCP from the hero/team photo,
				...
				foreach ( $needles as $needle ) {
					if ( false !== stripos( $blob, $needle ) ) {
						$is_lcp = true;
						break;
					}
				}
				if ( ! $is_lcp && ! $lcp_set && ! $is_home && ! $is_team_page && preg_match( '/\b(?:src|data-lazy-src|data-src)=(["\'])[^"\']*\/wp-content\/uploads\/[^"\']+\1/i', $blob ) ) {
					if ( ! preg_match( '/\b(?:width|height)=(["\'])(?:1[0-8]|[1-9])\1/i', $blob ) ) {
						$is_lcp = true;
					}
				}
				if ( $is_logo ) {
				...
				if ( $is_lcp && ! $lcp_set ) {
					...
					$lcp_set = true;
					// Drop noscript twin — image is no longer lazy-loaded.
					return $tag;
				}

				$tag = preg_replace( '/\sfetchpriority=(["\'])[^"\']*\1/i', '', $tag );
				if ( ! preg_match( '/\bloading=/i', $tag ) && preg_match( '/\b(?:src|data-lazy-src|data-src)=(["\'])[^"\']*\/wp-content\/uploads\/[^"\']+\1/i', $blob ) ) {
					$tag = preg_replace( '/<img\b/i', '<img loading="lazy"', $tag, 1 );
				}
				return $tag . ( $noscript_img ? '<noscript>' . $noscript_img . '</noscript>' : '' );
			},
	...
	/**
	 * Promote data-lazy-src / data-src / noscript twin to src and drop Rocket lazy placeholders.
	 *
	 * @param string $tag          Single img tag.
	 * @param string $noscript_img Optional <img> from following <noscript>.
	 * @return string
	 */
	private static function unlazy_img_tag( $tag, $noscript_img = '' ) {
		...
		foreach ( array( 'data-lazy-src', 'data-src', 'data-rocket-src' ) as $attr ) {
			if ( preg_match( '/\b' . preg_quote( $attr, '/' ) . '="([^"]*)"/i', $tag, $m )
				|| preg_match( '/\b' . preg_quote( $attr, '/' ) . "='([^']*)'/i", $tag, $m ) ) {
				$real = $m[1];
				break;
			}
		}
		if ( '' === $real && is_string( $noscript_img ) && '' !== $noscript_img ) {
			if ( preg_match( '/\bsrc="([^"]*)"/i', $noscript_img, $m )
				|| preg_match( "/\bsrc='([^']*)'/i", $noscript_img, $m ) ) {
				$real = $m[1];
			}
			if ( preg_match( '/\bsrcset="([^"]*)"/i', $noscript_img, $m )
				|| preg_match( "/\bsrcset='([^']*)'/i", $noscript_img, $m ) ) {
				$srcset = $m[1];
			}
			if ( preg_match( '/\bsizes="([^"]*)"/i', $noscript_img, $m )
				|| preg_match( "/\bsizes='([^']*)'/i", $noscript_img, $m ) ) {
				$sizes = $m[1];
			}
		}
		// Placeholder SVG/data URI left behind after Rocket lazy — replace with real URL.
		if ( '' !== $real ) {
			if ( preg_match( '/\bsrc="/i', $tag ) ) {
				$tag = preg_replace( '/\bsrc="[^"]*"/i', 'src="' . esc_attr( $real ) . '"', $tag, 1 );
			} elseif ( preg_match( "/\bsrc='/i", $tag ) ) {
				$tag = preg_replace( "/\bsrc='[^']*'/i", "src='" . esc_attr( $real ) . "'", $tag, 1 );
			} else {
		...
		foreach ( array( 'data-lazy-src', 'data-src', 'data-rocket-src', 'data-lazy-srcset', 'data-lazy-sizes' ) as $attr ) {
			$tag = preg_replace( '/\s' . preg_quote( $attr, '/' ) . '="[^"]*"/i', '', $tag );
			$tag = preg_replace( '/\s' . preg_quote( $attr, '/' ) . "='[^']*'/i", '', $tag );
		}
		if ( ! preg_match( '/\bsrcset=/i', $tag ) ) {
			if ( '' !== $srcset ) {
				$tag = preg_replace( '/<img\b/i', '<img srcset="' . esc_attr( $srcset ) . '"', $tag, 1 );
			} elseif ( preg_match( '/\bdata-lazy-srcset="([^"]*)"/i', $tag, $sm ) || preg_match( "/\bdata-lazy-srcset='([^']*)'/i", $tag, $sm ) ) {
				$tag = preg_replace( '/<img\b/i', '<img srcset="' . esc_attr( $sm[1] ) . '"', $tag, 1 );
			}
		}
		if ( ! preg_match( '/\bsizes=/i', $tag ) ) {
			if ( '' !== $sizes ) {
				$tag = preg_replace( '/<img\b/i', '<img sizes="' . esc_attr( $sizes ) . '"', $tag, 1 );
			} elseif ( preg_match( '/\bdata-lazy-sizes="([^"]*)"/i', $tag, $sz ) || preg_match( "/\bdata-lazy-sizes='([^']*)'/i", $tag, $sz ) ) {
				$tag = preg_replace( '/<img\b/i', '<img sizes="' . esc_attr( $sz[1] ) . '"', $tag, 1 );
			}
		}
	 ...
	 * - on front pages (EN/RU/ZH): remove BreadcrumbList entirely
	 *   (GSC: "itemListElement missing" on ?lang=ru / ?lang=zh-hans)
	 * - never keep a BreadcrumbList without itemListElement
	 *
	 * @param array $graph Schema graph pieces.
	 * @return array
	 */
	private static function ensure_breadcrumb_in_graph( $graph ) {
		...
		// Homepages: breadcrumbs are not useful and WPML/Yoast stubs triggered GSC errors.
		...
		if ( $bc_indexes ) {
			...
			return self::sync_webpage_breadcrumb_refs( array_values( $graph ) );
		}
		...
		return self::sync_webpage_breadcrumb_refs( $graph );
	}
	 ...
	 * @param array|null $bc_indexes Indexes of BreadcrumbList nodes (optional).
	 * @return array
	 */
...
JS;
	...
	private static function fix_header_logo_html( $html ) {
		...
		// Rocket SVG / 300×100 attrs in the logo slot → real 44×44 (CLS).
		$logo_img = '<img src="' . $logo . '" width="44" height="44" alt="Cindemir Law Office" decoding="async" loading="eager">';
		$next     = preg_replace(
			'#(<span[^>]*\bclass=([\'"])[^\'"]*\blogo\b[^\'"]*\2[^>]*>\s*<a\b[^>]*>)\s*<img\b[^>]*>\s*(?:<noscript>\s*<img\b[^>]*>\s*</noscript>)?#is',
			'$1' . $logo_img,
			$html,
			1,
			$count
		...
		$html = preg_replace_callback(
			...
			static function ( $m ) use ( $logo ) {
				...
				return '<img src="' . $logo . '" width="44" height="44" alt="Cindemir Law Office" decoding="async" loading="eager"' . $extra . '>';
			},
			$html
		);
		if ( ! is_string( $html ) ) {
...
public static function homepage_hero_styles() {
	...
	public static function header_brand_styles() {
		...
		echo '<style id="cindemir-header-brand">'
			/* Meta bar is hidden — lock header to 64px. Competing cls-fix CSS uses 98px +
			 * height:auto here was the PSI .av-logo-container CLS 0.102 culprit. */
			. 'html{--cindemir-header-h:64px}'
			. '#top #header{'
			. 'min-height:64px!important;height:64px!important;max-height:64px!important;'
			...
			. 'text-decoration:none!important;min-height:44px!important;height:44px!important;'
			. 'max-height:44px!important;min-width:0!important}'
			...
			. 'max-width:min(240px,28vw)!important;overflow:hidden!important;text-overflow:ellipsis!important}'
			...
			. 'padding-top:var(--cindemir-header-h,64px)!important;margin-top:0!important}'
	...
	public static function language_switcher_boot_script() {
		...
		echo '<script id="cindemir-lang-switch" data-nowprocket nowprocket data-no-minify="1" data-no-optimize="1">'
			...
			/* Header height is CSS-fixed (--cindemir-header-h:64px). Do not measure/rewrite after paint. */
			. '</script>' . "\n";
	}
