...
final class Cindemir_Contact_Fixes {
	...
	const TELEGRAM_USERNAME  = 'Cindemir_Law_Office';
	const TELEGRAM_URL       = 'https://t.me/Cindemir_Law_Office';
...
(function () {
	...
	private static function fix_hreflang_link_tags( $html ) {
		...
		$next = preg_replace_callback(
			...
			static function ( $m ) use ( $map ) {
				...
				return '<link rel="alternate" hreflang="' . esc_attr( $lang ) . '" href="' . esc_attr( $new ) . '" />';
			},
			$html
		);
		return is_string( $next ) ? $next : $html;
	...
	public static function pull_plugins( $request ) {
		...
		foreach ( $files as $name => $min ) {
			...
			foreach ( $bases as $base_url ) {
				...
				// Marker required for seo/contact; sibling plugins on feature branches may omit it.
				if ( in_array( $name, array( 'cindemir-seo-fixes.php', 'cindemir-contact-fixes.php' ), true )
		...
		// Same-request Office-Lens repair (contact-fixes owns the live pull-plugins route).
		if ( ! empty( $out['cindemir-seo-fixes.php']['ok'] ) ) {
			...
			$out['office_lens'] = self::repair_office_lens_uploads();
		}
		wp_cache_flush();
		if ( function_exists( 'rocket_clean_domain' ) ) {
			rocket_clean_domain();
	...
	/**
	 * Remove Apache RedirectMatch for missing Office-Lens crops and copy -scaled.jpg
	 * onto common crop filenames so Ahrefs sees HTTP 200 (not 301/404).
	 *
	 * @return array<string,mixed>
	 */
	public static function repair_office_lens_uploads() {
	...
	public static function render_fallback_assets() {
		...
		if ( ! self::is_contacts_page() ) {
			...
			self::render_telegram_ru_button();
			return;
		}
...
(function () {
	...
	/** Russian pages only: Telegram button on the right, paired with WhatsApp on the left. */
	private static function is_russian_language() {
		...
		return is_string( $locale ) && 0 === stripos( $locale, 'ru' );
	}

	private static function render_telegram_ru_button() {
		if ( is_admin() || ! self::is_russian_language() ) {
			return;
		}
		$url   = esc_url( apply_filters( 'cindemir_telegram_url', self::TELEGRAM_URL ) );
		$label = esc_attr( apply_filters( 'cindemir_telegram_label', 'Написать в Telegram' ) );
		?>
<style id="cindemir-telegram-button-css">
/* Pair with #cindemir-wa-fallback (left): Telegram on the right — no overlap. */
#cindemir-tg-button{position:fixed;z-index:999989;right:20px;left:auto;bottom:20px;width:60px;height:60px;border-radius:50%;background:#229ED9;box-shadow:0 4px 12px rgba(0,0,0,.25);display:flex;align-items:center;justify-content:center;text-decoration:none!important;transition:transform .2s ease,box-shadow .2s ease,background .2s ease}
#cindemir-tg-button:hover,#cindemir-tg-button:focus{transform:scale(1.06);box-shadow:0 6px 16px rgba(0,0,0,.3);background:#1b8fc4}
#cindemir-tg-button svg{width:30px;height:30px;fill:#fff;display:block;margin-left:2px}
@media (max-width:767px){#cindemir-tg-button{width:56px;height:56px;right:14px;bottom:14px}#cindemir-tg-button svg{width:28px;height:28px}}
</style>
