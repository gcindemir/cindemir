...
final class Cindemir_Contact_Fixes {
	...
	public static function boot() {
		...
		add_filter( 'rocket_buffer', array( __CLASS__, 'enhance_page_html' ), 16 );
		...
		add_filter( 'rocket_exclude_defer_js', array( __CLASS__, 'debloat_exclusions' ) );
		...
		add_action( 'wp_footer', array( __CLASS__, 'render_footer_socket_extras' ), 99 );
	...
	/** Inject footer into final HTML (runs inside WP Rocket buffer before cache write). */
	public static function enhance_page_html( $html ) {
		...
		$html = self::linkify_copyright_html( $html );
		$html = self::inject_socket_extras_html( $html );
		return $html;
	}
	...
	private static function inject_socket_extras_html( $html ) {
...
<style id="cindemir-footer-fixes-css">#socket .container{text-align:center!important}#socket .copyright{float:none!important;display:block;width:100%;text-align:center!important;margin:0 auto}#socket .cindemir-socket-extras{width:100%;margin-top:14px;padding-top:12px;border-top:1px solid rgba(255,255,255,.15);text-align:center}#socket .cindemir-baro-verification-bar{margin-bottom:12px;text-align:center}#socket .cindemir-baro-verification-bar a{color:inherit;text-decoration:underline;font-size:14px}#socket .cindemir-footer-social{margin:10px 0 14px}#socket .cindemir-footer-social-list{display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;margin:0;padding:0;list-style:none}#socket .cindemir-footer-social-list a{display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,.12);color:#fff;text-decoration:none}#socket .cindemir-footer-social-list svg{width:18px;height:18px;fill:currentColor;display:block}#socket .cindemir-footer-badges{display:flex;align-items:center;justify-content:center;gap:14px;flex-wrap:wrap}#socket .cindemir-footer-badges img{height:48px;width:auto;display:block;object-fit:contain;border:0}#socket .copyright a.cindemir-footer-email,#socket .copyright a.cindemir-footer-phone{color:inherit;text-decoration:underline}</style>
	...
	public static function rest_purge_cache( $request ) {
		...
		self::purge_all_caches();
		update_option( 'cindemir_contact_fixes_version', '1.3.4', false );
...
(function () {
	...
	public static function render_footer_socket_extras() {
		if ( is_admin() ) {
			return;
		}
		$extras = self::socket_footer_extras_markup();
		?>
<script id="cindemir-footer-live" data-no-defer="1" data-no-minify="1">
(function(){
  ...
  if(c && c.innerHTML.indexOf('cindemir-footer-email')<0){
    c.innerHTML=c.innerHTML.replace(ph,'<a href="tel:+902165506775" class="cindemir-footer-phone">'+ph+'</a>').replace(em,'<a href="mailto:'+em+'" class="cindemir-footer-email">'+em+'</a>');
  }
  if(document.getElementById('cindemir-socket-extras')) return;
  var box=document.querySelector('#socket .container');
  if(!box) return;
  box.insertAdjacentHTML('beforeend', <?php echo wp_json_encode( $extras ); ?>);
})();
</script>
		<?php
	}
