...
final class Cindemir_Menu_Fix {
	...
	/**
	 * Local press page URL for the active (or given) language.
	 *
	 * @param string|null $lang Language code.
	 * @return string
	 */
	private static function press_url( $lang = null ) {
		$lang = $lang ? strtolower( (string) $lang ) : self::front_lang();
		if ( 'ru' === $lang ) {
			return home_url( '/press/?lang=ru' );
		}
		if ( in_array( $lang, array( 'zh-hans', 'zh' ), true ) ) {
			return home_url( '/press/?lang=zh-hans' );
		}
		return home_url( '/press/' );
	}
	...
	public static function maybe_repair_menus() {
		// v1.6.6: press items must point at local /press/ (not cindemir.av.tr).
		if ( get_option( 'cindemir_menu_fix_v1' ) === '1.6.6' ) {
			return;
		}
		...
		self::repair_ru_menu();
		self::repair_zh_menu();
		update_option( 'cindemir_menu_fix_v1', '1.6.6', false );
	}
		...
		if ( get_post( 2678 ) ) {
			wp_update_nav_menu_item(
				...
				array(
					...
					'menu-item-url'    => self::press_url( 'ru' ),
					'menu-item-status' => 'publish',
				)
			);
		}
		...
		if ( get_post( 2673 ) ) {
			wp_update_nav_menu_item(
				...
				array(
					...
					'menu-item-type'   => 'custom',
					'menu-item-url'    => self::press_url( 'zh-hans' ),
					'menu-item-status' => 'publish',
				)
			);
		}
	...
	public static function fix_menu_objects( $items, $args ) {
		...
		foreach ( $items as $item ) {
			if ( empty( $item->type ) || 'post_type' !== $item->type || empty( $item->object_id ) ) {
				// Press custom link.
				if ( ! empty( $item->url ) && ( false !== strpos( $item->url, '/press' ) || false !== strpos( $item->url, 'we-are-in-news' ) || false !== strpos( $item->url, 'basinda-biz' ) || false !== strpos( (string) $item->title, 'Press' ) || false !== strpos( (string) $item->title, 'пресс' ) || false !== strpos( (string) $item->title, '媒体' ) || false !== strpos( (string) $item->title, '支持' ) ) ) {
					$item->url   = self::press_url( $lang );
	...
	/** Point legacy cindemir.av.tr press URLs at the local press page. */
	private static function rewrite_press_hrefs( $html ) {
		if ( ! is_string( $html ) || '' === $html ) {
			return $html;
		}
		$local = self::press_url();
		$html  = preg_replace(
			'#\bhref=(["\'])https?://(?:www\.)?cindemir\.av\.tr/(?:en/we-are-in-news|basinda-biz)/?(?:\?[^"\']*)?\1#i',
			'href=$1' . esc_attr( $local ) . '$1',
			$html
		);
