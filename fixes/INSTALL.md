...
**Önerilen — WordPress eklenti yükleyici (tek tık):**

1. `fixes/deploy-package/cindemir-deploy-helper.zip` indir
2. wp-admin → Eklentiler → Yeni Ekle → Eklenti Yükle → zip seç → Yükle
3. **Etkinleştir** — eklenti GitHub'dan tüm mu-plugin dosyalarını geri yükler ve cache temizler
4. WP Rocket cache temizlendiğini doğrula; mobilde EN/TR/RU/ZH kontrol et

**Alternatif — File Manager / FTP:**

| File | Target |
|------|--------|
| `fixes/deploy-package/mu-plugins.zip` | `wp-content/mu-plugins/` (extract) |
| veya tek tek: | |
| `fixes/mu-plugins/cindemir-seo-fixes.php` | `wp-content/mu-plugins/` (v1.7.2) |
| `fixes/mu-plugins/cindemir-expose-yoast-meta.php` | `wp-content/mu-plugins/` |
| `fixes/mu-plugins/cindemir-mobile-header-branding.php` | `wp-content/mu-plugins/` (mobil header) |
| `fixes/mu-plugins/cindemir-contact-fixes.php` | `wp-content/mu-plugins/` |
| `fixes/mu-plugins/cindemir-mobile-brand.php` | `wp-content/mu-plugins/` (yedek) |

**Agent otomasyonu için** Cursor secrets:
